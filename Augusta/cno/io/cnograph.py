    ...
    **degree**. You can then call plot as follows to replace the default
    ...
    def __init__(self, model=None, data=None, verbose=False, **kargs):
        ...
        if hasattr(model, '__class__') and \
            ...
            if model.endswith('.sif'):
                self.read_sif(model)
                self.filename = model[:]
            elif model.endswith(".xml") or model.endswith(".sbml"):
                self.read_sbmlqual(model)
                self.filename = model[:]
            else:
                raise CNOError("Only filenames with .sif and .xml / .sbml (SBML-qual) extension are recognised.")
        ...
        if "+" in str(u):
            ...
            for x in lhs:
                if x.startswith("!"):
                    ...
                    attr['color'] = 'red'
                    attr['arrowhead'] = 'tee'
                    super(CNOGraph, self).add_edge(x[1:], v, **attr) # super(CNOGraph, self).add_edge(x, v, attr_dict, **attr)
                else:
                    attr["link"] = "+"
                    super(CNOGraph, self).add_edge(x, v, **attr) # super(CNOGraph, self).add_edge(x, v, attr_dict, **attr)
        else:
            attr = self.get_default_edge_attributes(**attr)
            super(CNOGraph, self).add_edge(u, v, **attr) # super(CNOGraph, self).add_edge(x, v, attr_dict, **attr)
    ...
    def _nodes2reac(self, inputsNodes, output):
        ...
        for node in inputsNodes:
            if self.edge[node][output]['link']=="-": # if self.edge[node][output]['link']=="-":
